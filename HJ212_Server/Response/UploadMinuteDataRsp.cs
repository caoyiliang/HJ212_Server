...
{
    internal class UploadMinuteDataRsp : IAsyncResponse_Server<(DateTime dataTime, List<StatisticsData> data, RspInfo RspInfo)>, IRspEnumerable
    {
        ...
        public async Task AnalyticalData(string clientInfo, byte[] bytes)
        {
            ...
            {
                throw new ArgumentException($"GB_Server HJ212 UploadMinuteData DataTime Error");
            }
        ...
        public (bool Type, byte[]? CheckBytes) Check(string clientInfo, byte[] bytes)
        {
            ...
            return (rs.Where(item => item.Contains($"CN={(int)CN_Client.上传污染物分钟数据}")).Any(), default);
        }
