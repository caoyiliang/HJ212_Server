...
{
    internal class GetSampleExtractionTimeRsp : IAsyncResponse_Server<(int Stime, RspInfo RspInfo)>, IRspEnumerable
    {
        private int _stime;
        ...
        public async Task AnalyticalData(string clientInfo, byte[] bytes)
        {
            ...
            if (!int.TryParse(dataList.SingleOrDefault(item => item.Contains("Stime"))?.Split('=')[1], out _stime))
            {
                throw new ArgumentException($"GB_Server HJ212 GetSampleExtractionTime Stime Error");
            }
        ...
        public (bool Type, byte[]? CheckBytes) Check(string clientInfo, byte[] bytes)
        {
            ...
            return (rs.Where(item => item.Contains($"CN={(int)CN_Client.上传出样时间}")).Any(), qn == null ? default : Encoding.ASCII.GetBytes(qn));
        }

        public (int Stime, RspInfo RspInfo) GetResult()
        {
            return (_stime, _rspInfo);
        }
